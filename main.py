...
║               نسخة مخصصة للاستضافة السحابية (Render)             ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
# تطبيق nest_asyncio لتجنب مشاكل الـ Event Loop مع Asyncio في بعض البيئات
import nest_asyncio
nest_asyncio.apply()
...
# ══════════════════════════════════════════════════════════════════════════════
#  ⚙️  الإعدادات وقراءة متغيرات البيئة
# ══════════════════════════════════════════════════════════════════════════════
load_dotenv()  # قراءة ملف .env محلياً إن وجد
...
if not BOT_TOKEN or not OPENROUTER_KEY:
    print("⚠️ تحذير: لم يتم العثور على BOT_TOKEN أو OPENROUTER_KEY في متغيرات البيئة!")
...
async def analyze_image(file_url: str, pair: str, expiry_label: str) -> str:
    prompt = (
        f"أنت خبير تداول بالخيارات ثنائية OTC على PocketOption.\n"
...
HOME_TEXT = (
    ...
    "🧠 14 استراتيجية احترافية مخصصة OTC\n"
...
async def cmd_grant(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    args = ctx.args or []
    if len(args) < 2:
        await update.message.reply_text("الاستخدام: /grant <uid> <أيام>")
        return
    try:
        uid  = int(args[0])
        days = int(args[1])
...
async def cmd_revoke(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    args = ctx.args or []
    if not args:
        await update.message.reply_text("الاستخدام: /revoke <uid>")
        return
    try:
        uid = int(args[0])
        revoke_access(uid)
...
# ─── خادم ويب خفيف لإبقاء الخدمة تعمل على Render ─────────────────────────────────
...
def run_web_server():
    # Render يمرر تلقائياً المنفذ في متغير بيئة PORT
...
def main() -> None:
    ...
    # تشغيل خادم الويب في خلفية Thread منفصل
